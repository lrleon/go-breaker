...
func AddEndpointToRouter(router *gin.Engine, breakerAPI *BreakerAPI) {
	...
	group.POST("/set_memory/:threshold", breakerAPI.SetMemory)
	group.POST("/set_latency/:threshold", breakerAPI.SetLatency)
	group.POST("/set_latency_window_size/:size", breakerAPI.SetLatencyWindowSize)
	group.POST("/set_percentile/:percentile", breakerAPI.SetPercentile)
	group.POST("/set_wait/:wait_time", breakerAPI.SetWait)
